...
## Run it
Open `index.html` — no server, no build, no CDN. Use the **Dental / Veterinary** toggle to
switch brands; the header logo sub-label, utility links, category nav + flyouts, hero,
catalog, manufacturers and footer legal all swap. Hover a category to open its flyout.
The cart-count and toast behaviour was JavaScript-only and is not part of the static kit.

## Composition
- `brands.js` — brand config (labels, nav, flyout categories, hero copy, products, legal) for `dental` and `vet`.
- `StoreHeader.jsx` (React source, not loaded by `index.html`) — navy utility bar + white main bar (real Patterson mark, Algolia-style search, Sign In, cart) + category nav with hover flyout.
- `StoreHome.jsx` (React source) — hero, "where to start" quick links, service banner, new & featured products, featured manufacturers, Advantage Rewards, testimonial, create-account CTA.
- `StoreFooter.jsx` (React source) — multi-column links, HQ, legal.
- `icons.jsx` — Lucide-style inline icons.
- Components come from `ds/components/components.css` as plain classes: **`.pat-btn`,
  `.pat-card`, `.pat-badge`, `.pat-input`**. `index.html` is static HTML + CSS — the brand
  toggle and the category flyouts are CSS-only (`:checked` / `:hover`), with no JavaScript.

## Fidelity notes
- **Font:** the real sites render in **proxima-nova** (Adobe Fonts). This kit uses the system
  `--font-sans` stack (`"proxima-nova", Arial, sans-serif` — Adobe Fonts kit `uth1qfm`).
